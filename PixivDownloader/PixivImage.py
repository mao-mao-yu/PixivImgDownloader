...
class ImageData:
    ...
    def get_image_data(self, image_id: str or int):
        """
        用re查找到元数据进行解析
        :return: 插画数据
        """
        ...
        illust_data = [
            illust_data['illustType'],
            illust_data.illustTitle,
            illust_data.pageCount,
            illust_data.urls[self.image_size]
        ]
        return illust_data

    def get_ugoira_data(self, image_id: str or int):
        image_id = str(image_id)
    ...
    def _get_cookie(self) -> str:
        ...
        :return: cookie
        """
        from commons import get_cookie
        path = os.path.split(self.cookie_path)[0]
        if not os.path.exists(path):
            os.makedirs(path)
        cookie = get_cookie.get_cookie(self.username, self.password)
